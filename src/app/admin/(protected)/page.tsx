...
export default async function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: "Upcoming", value: d.upcomingShows, color: "#d6b25e", href: "/admin/shows", urgent: false },
    { label: "Merch", value: d.activeMerch, color: "#a78bfa", href: "/admin/merch", urgent: false },
    { label: "Media", value: d.activeMedia, color: "#34d399", href: "/admin/media", urgent: false },
  ];
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {d.newCount > 0 && (
          <Link
            ...
            className="flex items-center gap-2 text-xs px-3 py-2 rounded-xl font-semibold shrink-0"
            style={{
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {statCards.map(({ label, value, color, href, urgent }) => (
          <Link
            key={label}
            href={href}
            className="rounded-2xl p-4 transition-all hover:scale-[1.015] group"
            style={{
              background: urgent ? "rgba(59,130,246,0.05)" : "rgba(18,18,20,0.8)",
          ...
          >
            ...
            <div className="text-xs mt-1.5 font-medium" style={{ color: "rgba(242,239,233,0.4)" }}>
              {label}
            </div>
          </Link>
        ))}
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
          {d.recentRequests.length === 0 ? (
            <div className="py-14 text-center">
              <p className="text-sm" style={{ color: "rgba(242,239,233,0.3)" }}>No requests yet.</p>
              <p className="text-xs mt-1" style={{ color: "rgba(242,239,233,0.18)" }}>New bookings will appear here.</p>
            </div>
          ) : (
            <>
              {/* Mobile: cards */}
              <div className="sm:hidden divide-y" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                {d.recentRequests.map((req) => (
                  <Link
                    ...
                    className="flex items-start justify-between gap-3 px-4 py-3.5 active:opacity-70"
                    style={{ background: "rgba(13,13,15,0.6)" }}
                  >
                    <div className="min-w-0">
                      <div className="font-medium text-sm truncate" style={{ color: "#f2efe9" }}>
                        {req.fullName}
                      </div>
                      <div className="text-xs mt-0.5 truncate" style={{ color: "rgba(242,239,233,0.4)" }}>
                        {req.eventType}
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      ...
                      <span className="text-[11px]" style={{ color: "rgba(242,239,233,0.3)" }}>
              ...
              <table className="hidden sm:table w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(0,0,0,0.18)" }}>
                    {["Client", "Event", "Budget", "Status", "Date", ""].map((h, i) => (
                      <th key={i} className="text-left px-5 py-3 text-[11px] font-semibold tracking-wide" style={{ color: "rgba(242,239,233,0.3)" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {d.recentRequests.map((req, i) => (
                    <tr
                      key={req.id}
                      style={{
                        borderBottom: i < d.recentRequests.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none",
                        background: "rgba(11,11,13,0.5)",
                      }}
                    >
                      <td className="px-5 py-3.5">
                        <div className="font-medium text-sm" style={{ color: "#f2efe9" }}>{req.fullName}</div>
                        <div className="text-xs mt-0.5" style={{ color: "rgba(242,239,233,0.35)" }}>{req.email}</div>
                      </td>
                      <td className="px-5 py-3.5 text-sm" style={{ color: "rgba(242,239,233,0.6)" }}>{req.eventType}</td>
                      <td className="px-5 py-3.5 text-xs" style={{ color: "rgba(242,239,233,0.4)" }}>
                        {req.budgetRange ?? "—"}
                      </td>
                      <td className="px-5 py-3.5"><StatusBadge status={req.status} /></td>
                      <td className="px-5 py-3.5 text-xs whitespace-nowrap" style={{ color: "rgba(242,239,233,0.35)" }}>
                        {new Date(req.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                      </td>
                      <td className="px-5 py-3.5">
                        <Link
                          ...
                          style={{ background: "rgba(214,178,94,0.08)", color: "#d6b25e", border: "1px solid rgba(214,178,94,0.15)" }}
                        >
                          Open
                        </Link>
                      </td>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="flex items-center gap-2 px-4 py-4 rounded-2xl text-sm font-medium transition hover:opacity-80"
            style={{
